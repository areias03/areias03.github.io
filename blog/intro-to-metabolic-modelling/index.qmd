---
title: "A (not so) short introduction to Metabolic Modelling"
description: "A practical introduction to concepts, methods and tools that will give you a better understanding of systems biology."
author:
  - name: Alexandre Areias Castro
date: "2025-04-14"
categories: [Python, Metabolic Modelling, Systems Biology, Metabolomics]
image: data/images/banner.jpg
format: 
  html:
    toc: true
...
- You are familiar with some concepts within systems biology and want to know more;
- Are looking for tools and methodologies to add to your worflows;
- Need some context to what some of these concepts actually mean.

> DISCLAIMER: This tutorial is a contextualization of the tutorials in
[embo_mcd22](https://github.com/cdanielmachado/embo_mcd22/tree/main) and
[MEWpy/examples](https://github.com/BioSystemsUM/MEWpy/tree/master/examples),
and therefore utlize the code written with them. The majority of the code here
presented is therefore not original, and should be creditted to the authors of
...
This is a fairly accurate answer, but leaves a lot of questions unanswered. Now, let's look at how we can *actually* do this.
...
[gapseq](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02295-1)
[@zimmermannGapseqInformedPrediction2021] (for gap-filling).
...
> WARNING: The output of the file is quite long. If you want to, you can go ahead and go through the various parameters and parts of the model. Otherwise, just jump into the next section using the Table of Contents on the right.
...
We will now test the *E. coli* ED1a model generated by CarveMe using the command below. We will start by reading it and then simulating microbial growth using **Flux-Balance Analysis (FBA)**:
...
Microbial communities are the natural occuring form of micrboes. To that end, if we want accurate predictions of microbial behaviour, it is important to represent them in conditions as close to those observed in nature as possible.
...
Unfortunately, one limitation of SteadyCom, which is exemplified by Chan, et al (2017) is the variability in the solution space when the community is not growing at the maximum (theoretical) growth rate.
